...
setwd("C:/Users/alekz/OneDrive/Escritorio/Tecnicas_Final_Agrupado/Oficial/Validacion/Tmax")
...
#####Correlacion
for (i in 1:length(obs$file)){
  cor_plot <- ggcorr(all_model[[i]][2:length(colnames(all_model[[i]]))], label = TRUE,method= c("all.obs", "spearman"))+ggtitle(paste0("Coef. Correlación Spearman - T° maxima Estación: ",obs$name[i]))+
    theme_bw()+
    theme(plot.title = element_text(hjust = 0.5,face = "bold"))
