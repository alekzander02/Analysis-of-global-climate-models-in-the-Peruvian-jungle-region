...
setwd("C:/Users/alekz/OneDrive/Escritorio/Tecnicas_Final_Agrupado/Nuevo Datos/Pisco/Precip")
...
Name_model <- substr(archivos,8,19)
Name_model
...
for (i in 1:length(obs$file)){
  ...
  for (j in 1:length(archivos)){
    model <- ReadNetCDF(archivos[j], vars ="pr", subset=list(lat=obs$lat[i],lon=obs$lon[i], time =c("1981-01-02", "2014-01-01")))
    if (j ==1){
      ini <- cbind(ini,model)
    }else{
  ...
  ini$`CMCC-ESM2_hi`  = ini$`CMCC-ESM2_hi`*86400
  ini$`CNRM-CM6-1_h` = ini$`CNRM-CM6-1_h`*86400
  ini$`EC-Earth3-CC` = ini$`EC-Earth3-CC`*86400
  mod_sta[[i]] <- ini
