...
setwd("C:/Users/alekz/OneDrive/Escritorio/Tecnicas_Final_Agrupado/Oficial/Validacion/Precip")
...
archivos <- list.files(pattern = ".*pr.*historical.*\\.nc$")
archivos

###Extrayendo nombres de modelos de los modelos listados
Name_model <- substr(archivos,8,20)
Name_model
...
for (i in 1:length(obs$file)){
  ...
  for (j in 1:length(archivos)){
    model <- ReadNetCDF(archivos[j], vars ="pr", subset=list(lat=obs$lat[i],lon=obs$lon[i], time =list(c("1981-01-02", "2014-01-01"))))
    if (j ==1){
      ini <- cbind(ini,model)
    }else{
  ...
  ini$`CMCC-ESM2_his` = ini$`CMCC-ESM2_his`*86400
  ini$`NorESM2-LM_hi` = ini$`NorESM2-LM_hi`*86400
  ini$`NorESM2-MM_hi` = ini$`NorESM2-MM_hi`*86400
  mod_sta[[i]] <- ini
  write.csv(mod_sta[[i]], file = paste0("model_hist_pp_",obs$name[i],".csv"), row.names = FALSE)
...
for (i in 1: length(obs$file)){
  ...
  write.csv(filt_station[[i]], file = paste0("obs_filt_hist_pp_",obs$name[i],".csv"), row.names = FALSE)
}
...
all_model <- list()
for (i in 1:length(obs$file)){
  all_model[[i]] <- data.frame(Fecha = filt_station[[i]]$Fecha,mod_sta[[i]][,4:length(colnames(mod_sta[[i]]))], Observado= filt_station[[i]]$Pp) #### Pp, esto se cambia por la variable a trabajar sea Tmin o Tmax
}
...
for (i in 1:length(obs$file)){
  graph <- ggplot(melt(all_model[[i]], id.vars = "Fecha",variable.name = "Fuente"), aes(x = Fecha, y = value)) +
    ...
    scale_color_manual(values = c("red", "blue", "green","black","violet","orange",
                                  "cyan4", "indianred", "magenta"))+ggtitle(paste0("Estación ",obs$name[i]," Precipitación 1981 - 2013"))+
    theme_bw()+
...
#####Correlacion
for (i in 1:length(obs$file)){
  cor_plot <- ggcorr(all_model[[i]][2:length(colnames(all_model[[i]]))], label = TRUE,method= c("all.obs", "spearman"))+ggtitle(paste0("Coef. Correlación de Spearman - Prec. Estación: ",obs$name[i]))+
    theme_bw()+
    theme(plot.title = element_text(hjust = 0.5,face = "bold"))
  ggsave(plot=cor_plot,file=paste0("cor_plot_pp_",obs$name[i],".png"), width = 7, height = 6)
...
RMSE
write.csv(RMSE, file = paste0("RMSE_hist_pp_.csv"), row.names = T)
...
BIAS
write.csv(BIAS, file = paste0("BIAS_hist_pp_.csv"), row.names = T)
...
MAE
write.csv(MAE, file = paste0("MAE_hist_pp_.csv"), row.names = T)
